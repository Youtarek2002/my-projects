...
void xml(){
       ...
       while((y=fgetc(fp))!=EOF)
   {
    ...
    highno=checkop(a,in5,in6);}
     if(fp==NULL||row==-1||column==-1||highno==-1||x==-1||z==-1||in2<in1||in4<in3||in6<in5||x>in1||z<in6||row<4||column<4||highno<3)
    {    system("cls");
         system("COLOR 05");
        printf("\033[;31m");
        printf("The XML file is corrupted or the parameters may be too small!!!\n");
        for(i=0;i<3;i++)
        {
